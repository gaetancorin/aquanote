...
function changeNameAqua(array $input){
    ...
    if (!isset($input['new_name_aquarium']) || $input['new_name_aquarium'] === ''){
        throw new Exception('Vous devez indiquez un nom pour renommer l\'aquarium'.$aquarium_connected->name_aquarium);
    }
    $new_name_aquarium = $input['new_name_aquarium'];

    // check que le nouveau nom est différent de l'ancien
    if($new_name_aquarium === $aquarium_connected->name_aquarium){
        throw new Exception('Pour changer le nom de "'.$aquarium_connected->name_aquarium.'", vous devez indiquez un autre nom');
    }

    // Changement du nom de l'aquarium
    try{
        $aquariumRepository->updateNameAquariumById($new_name_aquarium, $aquarium_connected->id_aquarium);
    } catch(Exception){
